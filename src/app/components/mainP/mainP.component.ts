...
export class MainPComponent implements OnInit {
  ...
  commitPoint(username: string | null, r: number, x: number, y: number) {
    if (!this.authService.isTokenExpired() || localStorage.getItem("user") === null || this.currentUser !== localStorage.getItem("user") || this.currentUser !== localStorage.getItem("user") ||username !== localStorage.getItem("user")) {
      localStorage.clear();
  ...
  deletePoints(username: string) {
    if(!this.authService.isTokenExpired() || localStorage.getItem("user") === null || this.currentUser !== localStorage.getItem("user") ||username !== localStorage.getItem("user")){
      localStorage.clear();
      this.router.navigateByUrl("/~s284691/dist/ClientPart/start");
      alert("Вы что-то нашаманили")
      return;
    }
